...
# --- 1. POORA TEXT BLOCK NEECHE PASTE KARO ---
# (Wahi poora text jisme Swayam + Official dono hain)
full_text_data = """
...
# --- Output file name ---
output_filename = "quiz_data_output.txt"

# --- Helper function to parse correct answers ---
def get_correct_indices(options, correct_marker_line):
    indices = []
    # Clean up the marker line
    correct_marker_line = correct_marker_line.lower()\
                          .replace('(correct answer:', '')\
                          .replace('(correct option:', '')\
                          .replace(')', '').strip()

    # Handle cases like "a,b,c" or "a, b, c" or "a" or "b" or "b and c"
    # Split by common delimiters like ',', 'and'
    parts = re.split(r'[,\s]+and\s+|\s*,\s*', correct_marker_line)

    # Create option map based on actual options length: 'a'->0, 'b'->1 etc.
    option_map = {chr(ord('a') + i): i for i in range(len(options))}

    for part in parts:
        part = part.strip()
        if len(part) == 1 and 'a' <= part <= 'z':
            if part in option_map:
                indices.append(option_map[part])
        # Handle cases like "A,B" from official solutions if needed explicitly
        elif len(part) == 1 and 'A' <= part <= 'Z':
             part_lower = part.lower()
             if part_lower in option_map:
                 indices.append(option_map[part_lower])
...
parsed_questions = []
current_week = -1
current_source = "Unknown" # Still track source for parsing phase
lines = full_text_data.strip().split('\n')

i = 0
while i < len(lines):
    line = lines[i].strip()

    # Detect Headings for Week and Source
    week_match = re.search(r'Week (\d+)', line, re.IGNORECASE)
    if week_match:
        try:
            current_week = int(week_match.group(1))
        except ValueError:
             if "Week 0" in line: current_week = 0

    if "Swayam Interface" in line:
        current_source = "Swayam Interface"
    elif "Official Solution" in line or "Assignment-Week" in line: # Catch official headers
         current_source = "Official Solution"

    # Detect Question Start
    question_match = re.match(r"^\s*(\d+)\s*[).:]\s*(.*)", line, re.IGNORECASE)
    if not question_match:
        question_match = re.match(r"^\s*QUESTION\s*(\d+)\s*[:]\s*(.*)", line, re.IGNORECASE)

    if question_match:
        question_number_text = question_match.group(1)
        question_text_lines = []
        if question_match.group(2):
            question_text_lines.append(question_match.group(2).strip())

        options = []
        correct_indices_swayam = []
        official_answer_line = ""

        # Read lines for this question
        j = i + 1
        while j < len(lines):
            next_line = lines[j].strip()
            # Stop condition
            next_question_match = re.match(r"^\s*(\d+)\s*[).:]", next_line, re.IGNORECASE)
            if not next_question_match:
                 next_question_match = re.match(r"^\s*QUESTION\s*(\d+)\s*[:]", next_line, re.IGNORECASE)
            if next_question_match or next_line.startswith(('#', '---')):
                break

            # Check for options
            option_match = re.match(r"^\s*(✅)?\s*([a-dA-D])\s*[).]?\s*(.*)", next_line)
            if option_match:
                is_correct_swayam = bool(option_match.group(1))
                option_text = option_match.group(3).strip()
                current_option_index = len(options)
                options.append(option_text)
                if is_correct_swayam:
                    correct_indices_swayam.append(current_option_index)
            elif not options and next_line: # Part of question text
                question_text_lines.append(next_line)
            elif next_line.startswith('*(Correct Answer:') or next_line.startswith('*(Correct Option:'):
                official_answer_line = next_line
        ...
        if official_answer_line:
            correct_indices = get_correct_indices(options, official_answer_line)
        elif correct_indices_swayam:
            correct_indices = sorted(list(set(correct_indices_swayam)))

        # Handle Week 11 Official placeholder
        if current_source == "Official Solution" and current_week == 11:
             question_text = f"(Official Solution - Wk {current_week}, Q {question_number_text} Text Missing)"
             options = ["(Option Text Missing)"]
             q_num_local = int(question_number_text)
             if q_num_local == 1: correct_indices = [0]
             elif q_num_local == 2: correct_indices = [3]
             elif q_num_local == 3: correct_indices = [1]
             elif q_num_local == 4: correct_indices = [1]
             elif q_num_local == 5: correct_indices = [1]
             elif q_num_local == 6: correct_indices = [1]
             elif q_num_local == 7: correct_indices = [0, 1, 2]
             elif q_num_local == 8: correct_indices = [2]
             elif q_num_local == 9: correct_indices = [0]
             elif q_num_local == 10: correct_indices = [1]
             else: correct_indices = []

        if question_text and options and current_week != -1:
             question_text = re.sub(r"^\s*(\d+)\s*[).:]\s*", "", question_text).strip()
             question_text = re.sub(r"^\s*QUESTION\s*(\d+)\s*[:]\s*", "", question_text).strip()
...
for q in parsed_questions:
    grouped_by_week[q['week']].append(q)

final_quiz_data = []
for week_num in sorted(grouped_by_week.keys()):
    final_quiz_data.extend(grouped_by_week[week_num])

# --- Generate JavaScript Code String ---
# Use json.dumps for correct escaping and formatting
js_data_string = json.dumps(final_quiz_data, indent=4)
js_code_output = f"const quizData = {js_data_string};\n\n// --- END OF DATA ---"

# --- Save to text file ---
try:
    with open(output_filename, 'w', encoding='utf-8') as f:
        f.write("// --- PARSED QUIZ DATA ---\n")
        f.write("// <<< Yeh poora content copy karke script.js mein paste karo >>>\n\n")
        f.write(js_code_output)
    print(f"JavaScript data successfully '{output_filename}' file mein save ho gaya hai.")
